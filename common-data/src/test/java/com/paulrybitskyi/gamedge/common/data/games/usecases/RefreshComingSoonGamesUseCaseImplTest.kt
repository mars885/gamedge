...
import com.paulrybitskyi.gamedge.common.data.games.usecases.discovery.RefreshComingSoonGamesUseCaseImpl
...
internal class RefreshComingSoonGamesUseCaseImplTest {
    ...
    fun `Does not save remote games into local data store when refresh is unsuccessful`() {
        runTest {
            coEvery { throttler.canRefreshGames(any()) } returns false
            coEvery { gamesRemoteDataStore.getComingSoonGames(any()) } returns Err(
                DATA_ERROR_UNKNOWN
            )
