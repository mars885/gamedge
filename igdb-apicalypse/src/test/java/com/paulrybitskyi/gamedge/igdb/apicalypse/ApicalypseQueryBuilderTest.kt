...
import com.paulrybitskyi.gamedge.igdb.apicalypse.querybuilder.ApicalypseQueryBuilder
import com.paulrybitskyi.gamedge.igdb.apicalypse.querybuilder.ApicalypseQueryBuilderFactory
import org.junit.Assert.*
import org.junit.Before
import org.junit.Test

internal class ApicalypseQueryBuilderTest {


    private lateinit var queryBuilder: ApicalypseQueryBuilder


    @Before
    fun setup() {
        queryBuilder = ApicalypseQueryBuilderFactory.newBuilder()
    }


    @Test
    fun `Builds search query successfully`() {
        val query = queryBuilder.search("Shadow of the Colossus")
            .select("id, title")
            .offset(10)
            .limit(20)
            .sortDesc("id")
    ...
    fun `Builds select query successfully`() {
        val query = queryBuilder.select("id, title")
            .offset(100)
            .limit(500)
            .sortAsc("title")
            .build()
    ...
    fun `Builds select all query successfully`() {
        val query = queryBuilder.selectAll()
            .offset(100)
            .limit(500)
            .sortAsc("title")
            .build()
    ...
    fun `Builds select query where some condition is true`() {
        val query = queryBuilder.select("id, title, isLiked")
    ...
    fun `Builds select query where some condition is null`() {
        val query = queryBuilder.select("id, title, date")
            .where { "date".isNull }
            .offset(100)
            .limit(500)
            .sortAsc("title")
    ...
    fun `Builds select query where some field is larger than value`() {
        val query = queryBuilder.select("id, title, likes")
    ...
    fun `Builds select query where some field contains all of values`() {
        val query = queryBuilder.select("id, title, genres")
    ...
    fun `Builds select query with two conditions combined with logical and operator`() {
        val query = queryBuilder.select("id, title, genres, likes")
            .where {
