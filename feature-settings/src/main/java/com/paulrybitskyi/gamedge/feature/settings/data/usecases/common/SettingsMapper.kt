...
package com.paulrybitskyi.gamedge.feature.settings.data.usecases.common
...
internal class SettingsMapper @Inject constructor() {
    ...
    fun mapToDomainSettings(settings: DataSettings): DomainSettings {
        return DomainSettings(
            theme = DomainTheme.valueOf(settings.theme.name),
        )
    }

    fun mapToDataSettings(settings: DomainSettings): DataSettings {
        return DataSettings(
            theme = DataTheme.valueOf(settings.theme.name),
        )
    }
}
