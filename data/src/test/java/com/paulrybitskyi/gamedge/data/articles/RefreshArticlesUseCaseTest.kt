...
internal class RefreshArticlesUseCaseTest {
    ...
    private lateinit var refreshArticlesUseCase: RefreshArticlesUseCaseImpl
    ...
    fun setup() {
        ...
        refreshArticlesUseCase = RefreshArticlesUseCaseImpl(
    ...
    @Test
    fun `Emits remote articles when refresh is possible`() = runBlockingTest {
        ...
        assertEquals(
            ...
            refreshArticlesUseCase.execute(USE_CASE_PARAMS).first().get()
        )
    }


    @Test
    fun `Does not emit remote articles when refresh is not possible`() = runBlockingTest {
        throttler.canRefreshArticles = false

        var isEmptyFlow = false

        refreshArticlesUseCase.execute(USE_CASE_PARAMS)
    ...
    @Test
    fun `Saves remote articles into local data store when refresh is successful`() = runBlockingTest {
        throttler.canRefreshArticles = true
        articlesRemoteDataStore.shouldReturnArticles = true

        refreshArticlesUseCase.execute(USE_CASE_PARAMS).firstOrNull()
    ...
    @Test
    fun `Does not save remote articles into local data store when refresh is not possible`() = runBlockingTest {
        throttler.canRefreshArticles = false
    ...
    @Test
    fun `Does not save remote articles into local data store when refresh is unsuccessful`() = runBlockingTest {
        throttler.canRefreshArticles = false
    ...
    @Test
    fun `Updates articles last refresh time when refresh is successful`() = runBlockingTest {
        throttler.canRefreshArticles = true
        articlesRemoteDataStore.shouldReturnArticles = true
    ...
    @Test
    fun `Does not update articles last refresh time when refresh is not possible`() = runBlockingTest {
        throttler.canRefreshArticles = false
    ...
    @Test
    fun `Does not update articles last refresh time when refresh is unsuccessful`() = runBlockingTest {
        throttler.canRefreshArticles = false
        articlesRemoteDataStore.shouldReturnError = true
