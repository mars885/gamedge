...
internal class ObserveArticlesUseCaseTest {
    ...
    private lateinit var observeArticlesUseCase: ObserveArticlesUseCaseImpl
    ...
    fun setup() {
        ...
        observeArticlesUseCase = ObserveArticlesUseCaseImpl(
    ...
    @Test
    fun `Verify that articles are refreshed when refresh is requested`() = runBlockingTest {
        refreshArticlesUseCase.shouldReturnArticles = true

        observeArticlesUseCase.execute(USE_CASE_PARAMS)
    ...
    @Test
    fun `Emits articles from local data store when refresh is requested`() = runBlockingTest {
        refreshArticlesUseCase.shouldReturnArticles = true
        articlesLocalDataStore.saveArticles(DATA_ARTICLES)

        assertEquals(
            articleMapper.mapToDomainArticles(DATA_ARTICLES),
            observeArticlesUseCase.execute(USE_CASE_PARAMS).first()
        )
    }


    @Test
    fun `Emits articles from local data store when refresh use cases emits empty flow`() = runBlockingTest {
    ...
    @Test
    fun `Emits articles from local data store when refresh is not requested`() = runBlockingTest {
        articlesLocalDataStore.saveArticles(DATA_ARTICLES)

        assertEquals(
            articleMapper.mapToDomainArticles(DATA_ARTICLES),
            observeArticlesUseCase.execute(USE_CASE_PARAMS.copy(refreshArticles = false)).first()
        )
    }
    ...
    private class FakeRefreshArticlesUseCase : RefreshArticlesUseCase {
        ...
        override suspend fun execute(params: RefreshArticlesUseCase.Params): Flow<DomainResult<List<Article>>> {
            isExecuted = true
