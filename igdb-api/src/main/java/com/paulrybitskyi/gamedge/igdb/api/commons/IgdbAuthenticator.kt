...
internal class IgdbAuthenticator @Inject constructor(
    ...
    override fun authenticate(route: Route?, response: Response): Request? {
        ...
        return runBlocking {
            mutex.withLock {
                ...
                val lastSavedAccessToken = authLocalDataStore.getOauthCredentials()?.accessToken
    ...
    private suspend fun refreshCredentials(): OauthCredentials? {
        return authRemoteDataStore.getOauthCredentials().get()
            ?.also { credentials -> authLocalDataStore.saveOauthCredentials(credentials) }
    }
